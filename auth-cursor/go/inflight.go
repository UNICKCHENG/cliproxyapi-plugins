...
// inFlight tracks cancellable executor work so plugin.quiesce can drain it without stopping
...
// quiesce cancels in-flight executor work and parked tool runs. It does not stop bridges:
// reconfigure and shutdown own that.
func quiesce() {
	inFlight.mu.Lock()
	...
	abortParkedToolRuns(errors.New("plugin quiesce"))
	evictAllSessions()
}
